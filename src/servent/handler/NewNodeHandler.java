...
public class NewNodeHandler implements MessageHandler {
	...
	public void run() {
		if (clientMessage.getMessageType() == MessageType.NEW_NODE) {
			...
			boolean isMyPred = AppConfig.chordState.isKeyMine(String.valueOf(newNodeInfo.getChordId()));
			if (isMyPred) { //if yes, prepare and send welcome message
				...
				Map<String, SavedFile> myValues = AppConfig.chordState.getValueMap();
				Map<String, SavedFile> hisValues = new HashMap<>();
				
				int myId = AppConfig.myServentInfo.getChordId();
				int hisPredId = hisPred.getChordId();
				int newNodeId = newNodeInfo.getChordId();
				
				for (Entry<String, SavedFile> valueEntry : myValues.entrySet()) {
					String k = valueEntry.getKey();
					int key= ChordState.chordHash(k);
					if (hisPredId == myId) { //i am first and he is second
						if (myId < newNodeId) {
							if (key <= newNodeId && key > myId) {
								hisValues.put(k, valueEntry.getValue());
							}
						} else {
							if (key <= newNodeId || key > myId) {
								hisValues.put(k, valueEntry.getValue());
				...
				ServentInfo nextNode = AppConfig.chordState.getNextNodeForKey(String.valueOf(newNodeInfo.getChordId()));
